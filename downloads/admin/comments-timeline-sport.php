...
if(isset($_GET['delete']) && trim($_GET['delete'])){
	...
	if($Card->delsportsComment($idx)){
		C::redirect(C::link('comments-timeline-sport.php', false, true));
	}
}
if(isset($_GET['edit']) && trim($_GET['edit'])){
	$idx = $_GET['edit'];

	if($Card->confirmSportsComment($idx)){
		C::redirect(C::link('comments-timeline-sport.php', false, true));
	}
}
...
<section class="content">
	<section class="widget">
		...
		<?php
		$result = $User->query("SELECT `id`, `gdComments`, `badComments`, `sportsId`, `userId`, `rating`, `isRecommanded`, `updatedOn` FROM `tblSportsComment`");
				...
				<p><?php
					...
					<span class="reply">
						<?php if($value['isRecommanded'] == 'N'){?>
							<a href="<?php echo C::link('comments-timeline-sport.php', array('edit' => $value['id'] . '+news'), true);?>" style="color:#fff;background:green;padding:5px;">Verify</a>
						<?php }
						else{ ?>
							<a href="" style="color:#fff;background:green;padding:5px;">Verified</a>
						<?php } ?>
							<a href="<?php echo C::link('comments-timeline-sport.php', array('delete' => $value['id']), true);?>"  style="color:#fff;background:#ff0000;padding:5px;">Reject</a>
						</span>
				</p>
			</div>
		<?php

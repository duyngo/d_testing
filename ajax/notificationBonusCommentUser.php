...
$ids = array();
if(isset($_POST)){
	$res = $User->query("SELECT `id` FROM `tblBonusComment` WHERE `userId`='" . $_POST['id'] . "'");
	$list_response_id = array();
	if(is_array($res) && count($res) > 0) {
		foreach ($res as $idx => $val) {
			$ids = $val['id'];
			if (isset($ids) && count($ids) > 0) {
				$result = $User->query("SELECT * FROM `tblCommentResponse` WHERE `checkUser`='N' AND `isVerified`='Y' AND `category`='2' AND `responseId`='" . $ids . "'");
				...
				if (is_array($result) && count($result) > 0) {
					foreach ($result as $key => $value) {
						...
						if (is_array($res12) && count($res12) > 0) {
							...
							$res13 = $User->query("SELECT * FROM `tblBonusCards` WHERE `id` = '" . $value['categoryId'] . "'");
							if (is_array($res13) && count($res13) > 0) {
								foreach ($res13 as $key => $val13) {
									$sportsName = $val13['bonusName'];
								}

								echo '<a href="bonus-details/' . $val13["id"] . '/' . $sportsName . '">
		...
		if(is_array($res) && count($res) > 0) {
			foreach ($res as $idx => $val) {
				$res13 = $User->query("SELECT * FROM `tblBonusCards` WHERE `id` = '" . $val['bonusId'] . "'");
				if(is_array($res13) && count($res13) > 0){
					foreach ($res13 as $key => $val13) {

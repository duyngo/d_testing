...
if(isset($ids) && count($ids) > 0){
	$result = $User->query("SELECT `id`, `sportsId`, `userId`, `updatedOn` FROM `tblSadariSportsComment` WHERE `isRecommanded`='Y' AND `sportsId`='" . $ids . "'");//`checkSiteAdmin`='N' AND 
		if(is_array($result) && count($result) > 0){
			foreach ($result as $key => $value) {
				$result12 = $User->query("SELECT `id`, `userId` FROM `tblCommentResponse` WHERE `checkSiteAdmin`='N' AND `isVerified`='Y' AND `category`='3' AND `responseId`='" . $value['id'] . "' AND `categoryId`='" . $value['sportsId'] . "'");
					if(is_array($result12) && count($result12) > 0){
						foreach ($result12 as $index12 => $val12) {
							...
							$sports = $User->query("SELECT `id`, `sportsName` FROM `tblSadariCards` WHERE `id` = '" . $value['sportsId'] . "'");
								...
								echo 	'<a href="comment-reply.php?cat=sadari&id='.$value["id"].'">

	...
	if(is_array($res) && count($res) > 0){
		foreach ($res as $idx => $val) {
			...
			if(isset($ids) && count($ids) > 0){
				$result = $User->query("SELECT `id`, `categoryId`, `userId` FROM `tblCommentResponse` WHERE `checkUser`='N' AND `isVerified`='Y' AND `category`='1' AND `responseId`='" . $ids . "'");
					if(is_array($result) && count($result) > 0){
						foreach ($result as $key => $value) {
							...
							$res13 = $User->query("SELECT `id`, `sportsName` FROM `tblWebCards` WHERE `id` = '" . $value['categoryId'] . "'");

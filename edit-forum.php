...
if(isset($_POST) && is_array($_POST) && count($_POST) > 0 && $_POST['__formname__'] == 'POSTTOPICS'){
	// print_r($_POST);
	// print_r($_FILES);die();
    if($Forum->updateForumtopic($_POST, $_FILES)){
...
if(isset($_GET) && is_array($_GET) && count($_GET) > 0){
	...
	if($totals && is_array($totals) && count($totals) > 0){ //echo $totals[0]['id'];
		...
		$editValue['topicUniqueId'] = $totals[0]['topicUniqueId'];
		$editValue['topicTitle'] = $totals[0]['topicTitle'];
		$editValue['topicDescription'] = $totals[0]['topicDescription'];
		$editValue['topicFiles'] = $totals[0]['topicFiles'];
		$editValue['topicTags'] = $totals[0]['topicTags'];
				...
				<form action="" method="POST" id="postTopic" enctype="multipart/form-data">
					...
					<div class="other-informations">
						<?php 
							...
							$taglist = json_decode($editValue['topicTags']);

						?>
						<select name="topicTags[]" id="" class="select-tag" >
							<?php 
							$result = $User->query("SELECT `tagTitle`, `tagColor` FROM `tblTags`");
							foreach ($result as $tagKey => $tagValue) {
								$v = $tagValue['tagColor'].'@@'.$tagValue['tagTitle'];

							?>
							<option  value="<?php echo $v; ?>" <?php if(in_array($v, $taglist)){ echo " selected"; } ?>><?php echo $tagValue['tagTitle'];?></option>
							<?php
							}
							?>
						</select>

						<input type="text" name="topicTitle" class="input-title" placeholder="Discussion Title" value="<?php echo $editValue['topicTitle'];?>">
			    	...
			    	<textarea name="topicDescription" class="forum-editor" id="min-text" rows="15" style="width:100%;" ><?php echo $editValue['topicDescription'];?></textarea>
			    	<div class="forum-image-container" style="display:bolck;">
			    		<h5 class="text-danger">등록된 이미지</h5>
			    		<ul>
			    			<?php 
			    			$f = json_decode($editValue['topicFiles']); 

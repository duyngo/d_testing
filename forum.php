...
.forum-custom-group{
	color: #fff !important;
    background: rgba(0, 0, 0, 0) linear-gradient(to bottom, #1e262d 0%, #1e262d 60%, #213950 100%) repeat scroll 0 0;
	...
	<div class="row">
		<div class="col-lg-9 col-md-9 forum-list-container">
			<div class="cat-container">
				...
				<div class="all-cat collect-news">
				    <a href="<?php echo C::link('forum.php', array('tag' => 'Collected-post'), true);?>" class="btn" data-category="collect-news">먹튀사이트 취재</a>
				</div>
				<?php 
			...
			<div class="owl-carousel-tags tag-container">
				<div class="item">
					<a href="forum.php" class="btn" data-category=""><i class="fa fa-square" style="color:#fff;"></i> 전체보기 </a>
				</div>
				<?php 
				$result = $User->query("SELECT `id`, `tagTitle`, `tagColor`, `tagDescription`, `createdOn` FROM `tblTags` WHERE `isDispaly` = 'Y'");
			...
			<div class="betting-forum-list">
				<div class="forum-lists">
					<ul>
						...
						$category = (($_GET['category'] == 'content') ? "topicDescription" : "topicTitle"); 
						$filtersearch = (isset($_GET['search']) ? "AND `TFT`.`". $category ."` LIKE '%". $_GET['search'] ."%'" : " "); 
						//pagination
						$pagination = '';
						...
						if (isset($_GET['search'])) {
							...
							$resultopic = $User->query("SELECT SQL_CALC_FOUND_ROWS `TFT`.`id`, `TFT`.`topicUniqueId`, `TFT`.`topicTitle`, `TFT`.`topicTags`, `TFT`.`createdOn`, `TFT`.`isSticky`, `TU`.`userId`, `TU`.`nickName`, `TU`.`profile_img`, `TU`.`groupId`, `TU`.`createdOn` as `startedOn` FROM `tblForumTopics` as `TFT`, `tblUser` as `TU` WHERE `TFT`.`isDispaly` = 'N' AND `TFT`.`isVerified` = 'N' ". $filtertag . " AND `TFT`.`createdBy` = `TU`.`id`  ". $filtersearch . " ORDER BY `TFT`.`createdOn` DESC" . $pullSQL); // OR `TFT`.`topicDescription` LIKE '%". $_GET['search'] ."%'

						}else{
							$resultopic = $User->query("SELECT SQL_CALC_FOUND_ROWS `TFT`.`id`, `TFT`.`topicUniqueId`, `TFT`.`topicTitle`, `TFT`.`topicTags`, `TFT`.`createdOn`, `TFT`.`isSticky`, `TU`.`userId`, `TU`.`nickName`, `TU`.`profile_img`, `TU`.`groupId`, `TU`.`createdOn` as `startedOn` FROM `tblForumTopics` as `TFT`, `tblUser` as `TU` WHERE `TFT`.`isDispaly` = 'N' AND `TFT`.`isVerified` = 'N' ". $filtertag . " AND `TFT`.`createdBy` = `TU`.`id` ORDER BY `TFT`.`isSticky` ASC,  `TFT`.`createdOn` DESC" . $pullSQL);
							            	...
							            	<li class="item-terminalPost">
							            		<span>
						            				...
						            				<time pubdate="true" datetime="<?php echo $topicValue['createdOn']; ?>" title="<?php echo date_format($createdDate, DATE_COOKIE);?>" data-humantime="true">
														<?php
														if($diff->y > 0){
															echo 'posted '. $diff->y. ' 년 전';
														}else if($diff->m > 0){
															echo 'posted '. $diff->m. ' 달 전';
														}else if($diff->d > 0){
						            						echo 'posted '. $diff->d. ' 일 전';
														}else if($diff->h > 0){
						            						echo 'posted '. $diff->h. ' 시간 전';
														}else if($diff->i > 0){
						            						echo 'posted '. $diff->i. ' 분 전';
														}else{
															echo 'posted '. $diff->s. ' 초 전';
														}
														?>
						            				</time>
						            			</span>
						...
						<li>
							<div class="DiscussionListItem Slidable">
								<p style="padding:10px;">작성된 글이 없습니다.</p>
							</div>
						</li>
						<?php
						}
			...
			<div class="" style="margin:10px 0px;">
				...
				if(!isset($_GET['search'])){
					...
					<ul class="clearfix pagination pagination ask-pagination pull-left" style="margin-top: 0px;">
					    <li class=""><a href="forum.php?__pGI=1">« 처음</a></li>
						<?php 
						for ($i=1; $i<=$total_pages; $i++) {
					    echo '<li class="number '.$ac = (($page == $i) ? 'active disabled' : '') .'"><a data-pagenumber="'.$i.'" href="forum.php?__pGI='.$i.'">'.$i.'</a></li>';
						} ?>
					    <li class="copy next"><a href="forum.php?__pGI=<?php echo $total_pages; ?>">마지막 »</a></li>
					</ul>
				<?php 
						}
					} 
				...
				<form class="navbar-form navbar-left" method="GET" action="" style="margin-top: 0px;">
			      	...
			      	<button type="submit" class="btn btn-topic-search">검색</button> -->
			      	<div class="input-group">
				      <span class="input-group-addon forum-custom-group">
				        <label style="margin-bottom: 0px;"><input type="radio" name="category" value="title"> 제목</label>
				      </span>
				      <span class="input-group-addon forum-custom-group">
				        <label style="margin-bottom: 0px;"><input type="radio" name="category" value="content"> 내용</label>
				      </span>
				      <input type="text" class="form-control" placeholder="Search" name="search" style="color:#fff;background: rgba(0, 0, 0, 0.4) none repeat scroll 0 0;" />
				...
				<div class="all-cat start-discussion" style="float:right;">
				    <a href="javascript:void(0);" class="btn" data-category="start-discussion" data-check="<?php echo $lid; ?>" data-gid="<?php echo $gid; ?>">작성하기</a>
				</div>
			</div>
						...
						<form action="" method="POST" id="postTopic" enctype="multipart/form-data">
							...
							<div class="other-informations">
								<select name="topicTags[]" id="" class="select-tag" required >
									...
									<?php 
									if($lid == 1){
									$resultadmin = $User->query("SELECT `tagTitle`, `tagColor` FROM `tblTags` WHERE `isDispaly` = 'N'");
									foreach ($resultadmin as $tagadminKey => $tagadminValue) {
									?>
									<option value="<?php echo $tagadminValue['tagColor']. '@@' .$tagadminValue['tagTitle'];?>"><?php echo $tagadminValue['tagTitle'];?></option>
									<?php
									}
									}
									?>
									<?php 
									$result = $User->query("SELECT `tagTitle`, `tagColor` FROM `tblTags` WHERE `isDispaly` = 'Y'");
									foreach ($result as $tagKey => $tagValue) {
									?>
								...
								<input type="text" name="topicTitle" class="input-title" placeholder="제목을 입력해주세요.">
					    	...
					    	<div class="bottom-toolbar-forum">
					    		<ul>
					    			<li class=""><input type="file" name="topicFiles[]" id="topicFiles" ></li>
		...
		<div class="col-lg-3 col-md-3">
			<div class="information">
				<p>검증 토론을 이용하기 위해선 로그인 후, '등업신청' 부탁드립니다. * 오직 배팅타임을 통해 배팅사이트를 이용하고 있는 유저만 등업이 가능합니다.</p>
			</div>

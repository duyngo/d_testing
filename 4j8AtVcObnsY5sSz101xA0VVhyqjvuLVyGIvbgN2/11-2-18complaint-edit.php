			...
			<form action="" method="post" enctype="multipart/form-data">
				...
				<div class="field-wrap">
					...
					<p><span style="color:red;">Complaint  </span><?php echo $val['complaintText']; ?></p>
						...
						<div class="complaint-edit-previous">
					<?php
					...
					<?php if($response['siteName'] != ''){?>
						...
						<?php if($response['complaintFiles'] != ''){
							$filesComplaint = json_decode($response['complaintFiles']);
							if($filesComplaint != ''){
					...
					<span class="reply">
						<?php if(isset($groupId[0]['groupId']) == 0){ if($response['isVerified'] == 'N'){?>
							<a href="<?php echo C::link('complaint-edit.php', array('ed' => $response['id'] . '+verifyRespons+' .$val['id'] ), true);?>" style="color:#fff;background:green;padding:5px;">Verify</a>
						<?php }
						else{ ?>

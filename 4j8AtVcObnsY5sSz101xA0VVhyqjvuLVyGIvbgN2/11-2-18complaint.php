...
<section class="content">
	<section class="widget">
		...
		<?php
		$result = $User->query("SELECT `id`, `userId`, `siteName`, `link`, `reason`, `complaintTitle`, `complaintText`, `complaintFiles`, `onSiteAccountName`, `onSiteEmail`, `isVerified`, `updatedOn`, `status` FROM `tblComplaints` ORDER BY `updatedOn` ASC");
		...
		?>
			<div class="tl-post comments">
				...
				<p class="tl-post-p">
					...
					?>
					<strong><?php echo $val['userId']; ?></strong><span> &nbsp;&nbsp;&nbsp;&nbsp;(Playing website Account Name - <span style="color:red;"><?php echo $value['onSiteAccountName']; ?></span>,Phone Number - <span style="color:red;"><?php echo $value['onSiteEmail']; ?></span>)<br />
					<?php
						}
					}
					...
					<?php if($value['complaintFiles'] != '[]'){
						$filesComplaint = str_replace(array('[', ']'), '', $value['complaintFiles']);
						?>
							<a href="<?php echo $filesComplaint; ?>" target="_blank"><img src="<?php echo HOST.$filesComplaint; ?>" width="150px" height="90px"alt="" /></a>
					...
					<span class="reply">
					 ...
					 if(isset($groupId[0]['groupId']) == 0){
						...
						<?php }
							...
							<a href="javascript:void(0)" class="responseButton" style="color:#fff;background:orange;padding:5px;">Conversation</a>
						</span>
				</p>
				<div class="responseMsg">
					...
					<span class="reply">
						<?php if(isset($groupId[0]['groupId']) == 0){ if($response['isVerified'] == 'N'){?>
							<a href="<?php echo C::link('complaint.php', array('edit' => $response['id'] . '+verifyRespons+' . $value['id']), true);?>" style="color:#fff;background:green;padding:5px;">Verify</a>
						<?php }
						else{ ?>
							<a href="" style="color:#fff;background:green;padding:5px;">Verified</a>
						<?php

...
class User extends Base {
	...
	const USER_ONE = '3';
    const USER_NAME_ONE = 'USER level 1';

	const USER_TWO = '4';
    const USER_NAME_TWO = 'USER level 2';
                ...
                if($result = $this->query("SELECT `id`, `userId`, `isActive`, `emailValid` FROM `tblUser` WHERE `userId` = '" . trim($array['userId']) . "' AND `password` = '" . trim($array['password']) . "' AND `isDelete` = 'N'")){
                    if(is_array($result) && count($result) > 0){
                        if(isset($result[0]['emailValid']) && $result[0]['emailValid'] == 'N'){
                            ...
                            $this->query("INSERT INTO `tblUserSession` (`userId`, `phase`, `time`, `token`) VALUES ('" . $result[0]['id'] . "', 'LN', '" . SESSION_START_TIME . "', '" . md5(uniqid(rand(), true)) . "')");
    ...
    public static function token($id){
        $User = new User();
        if((int)$id > 0 && $result = $User->query("SELECT `token`, `time` FROM `tblUserSession` WHERE `userId` = '" . $id . "' AND `phase` = 'LN' ORDER BY `time` DESC LIMIT 0, 1")){
            if(is_array($result) && count($result) > 0){
                return $result[0];
            }
        }
   ...
   //              )));
	...
	public function getUserTypeArray(){
        return array(
            ...
            array(self::USER_ONE, self::USER_NAME_ONE),
            array(self::USER_TWO, self::USER_NAME_TWO)
        );
    }
    ...
    public function getDesignationArray(){
        return array(
            ...
            array(self::USER_NAME_ONE, self::USER_NAME_ONE),
            array(self::USER_NAME_TWO, self::USER_NAME_TWO)
        );
    }
    ...
    public function updateUser($array){
        if(!isset($array['nickName']) || trim($array['nickName']) == ''){
            ...
            $fieldArray = array(
                ...
                'groupId' => $array['groupId'],
                'siteName' => (is_string($array['adminSite']) != '' ? $array['adminSite'] : 'NULL'),
                'emailValid' => $array['emailValid'],
                'profile_img' =>  (is_string($array['profile_img']) != '' ? $array['profile_img'] : 'NULL'),
                'updatedBy' => $this->getLoggedInUserId(),
            );
        }
        //echo "UPDATE `tblUser` SET " . $this->prepareFieldsForInsertOrUpdate($fieldArray) . " WHERE `id` = '" . $array['id'] . "'";die();
        if($this->query("UPDATE `tblUser` SET " . $this->prepareFieldsForInsertOrUpdate($fieldArray) . " WHERE `id` = '" . $array['id'] . "'")){
            Message::addMessage($fieldArray['userId'] . " 성공적으로 처리되었습니다.", SUCCS);
        }
        return true;
    }

			...
			<div class="betting-forum-list">
				<div class="forum-lists">
					<ul>
						...
						$cnt = array();
						$idsr = $User->query("SELECT `topicUniqueId`, COUNT(`topicUniqueId`) AS `cnt` FROM `tblForumTopicsLikeDislike` WHERE `status` = 'Y' GROUP BY `topicUniqueId` ORDER BY `cnt` DESC");

						foreach ($idsr as $cntKey => $cntValue) {
							if($cntValue['cnt'] >= 2){
								array_push($cnt, $cntValue['topicUniqueId']);
							}
						}

						$resultopic = $User->query("SELECT SQL_CALC_FOUND_ROWS `TFT`.`id`, `TFT`.`topicUniqueId`, `TFT`.`topicTitle`, `TFT`.`topicTags`, `TFT`.`createdOn`, `TFT`.`isSticky`, `TU`.`userId`, `TU`.`nickName`, `TU`.`profile_img`, `TU`.`groupId`, `TU`.`createdOn` as `startedOn` FROM `tblForumTopics` as `TFT`, `tblUser` as `TU` WHERE `TFT`.`topicUniqueId` IN ('".implode("','",$cnt)."')  AND `TFT`.`isDispaly` = 'N' AND `TFT`.`isVerified` = 'N' AND `TFT`.`createdBy` = `TU`.`id` ORDER BY `TFT`.`isSticky` ASC,  `TFT`.`createdOn` DESC" . $pullSQL);
			...
			<div class="" style="margin:10px 0px;">
				...
				if(!isset($_GET['search'])){
					...
					<ul class="clearfix pagination pagination ask-pagination pull-left" style="margin-top: 0px;">
					    <li class=""><a href="best-post.php?__pGI=1">« 처음</a></li>
						<?php 
						for ($i=1; $i<=$total_pages; $i++) {
					    echo '<li class="number '.$ac = (($page == $i) ? 'active disabled' : '') .'"><a data-pagenumber="'.$i.'" href="best-post.php?__pGI='.$i.'">'.$i.'</a></li>';
						} ?>
					    <li class="copy next"><a href="best-post.php?__pGI=<?php echo $total_pages; ?>">마지막 »</a></li>
					</ul>
				<?php 
						}
					} 
				...
				?>
				<form class="navbar-form navbar-left" method="GET" action="forum.php" style="margin-top: 0px;">
